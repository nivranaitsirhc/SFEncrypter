...
if "%1" equ "translator" if exist %translator% (echo:viewing translator.. & notepad %translator% & exit /b) else (echo:translator does not exist..&exit /b 1)
set "error=0"
::-----------------------------------------------------------------------------------------------
set "switch_s=/d /s /u"
set "sw1_hit=0"
...
if "%encode%" equ "yes" (
	...
	if "!result!" equ "" echo:No code was entered Aborting..&exit /b 1
	set "code=!result!"
)
...
:recovery
	...
	if "%error%" neq "0" (echo:Errors were found during recovery^^!? Contact Christian Arvin ^(naitsirhc.uriel@gmail.com^) & %extd% /speak "Errors were found during Recovery. For help, Please Contact C.A." 2) else (del /q /f %translator% &echo:Done..^^!)
	( endlocal
