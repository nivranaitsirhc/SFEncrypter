...
if "%~1" equ "/ver"  (%extd% /speak "Secure File Encrypter version %version%" 2 &echo: Secure File Encrypter version.%version% & echo: Compiled By: Christian Arvin & exit /b)
if "%~1" equ "/?" (call :help & exit /b)
if "%~1" equ "/check" (call :check & exit /b)
if "%~1" equ "settings" (echo:viewing settings.. & notepad %settings% & exit /b)
if "%~1" equ "translator" (echo:viewing translator.. & notepad %translator% & exit /b)

if "%~1" equ "" (echo:Nada &exit /b) else (
	if "%~3" neq "" (
	  >&2 echo ERROR: Too many arguments
	  exit /b 1
	)
	if "%~1" equ "%2" (echo: Redundant Switch&exit /b 1)
...
if "!sw2_hit!" equ "1" (
	...
	if defined filter (
		>&2 echo:Only One filter allowed&exit /b 1
	) else (
		set "filter=%~2"
		set "switch=%~1"
	)
...
if exist %settings%( call :settings_op ) else (call :settings)
...
:: Parsing Switches
if "%switch%" equ "/u" (call :recovery error &exit /b) 
if "%switch%" equ "/s" (
...
if exist %translator% (
	if "%filter%" equ "add" (
		...
		::%extd% /speak "Warning!!. This Destination is already Encrypted. if you want to proceed, you need to first decrypt the files. Please enter yes, to proceed." 2
		echo:The destination is already encrypted
		echo:Enter yes if you want to continue:
		set /p ans=
		if "!ans!" equ "yes" (call :recovery error & if "!error!" neq "0" exit /b) else (exit /b)
	)
)
if "%encode%" equ "yes" (
	echo:Enter your AES key & %extd% /inputbox "AES Encoding" "Enter your AES key. Note! Keep your key safe." "%code%"
	if not "!result!" equ "" set "code=!result!" else exit /b
)
...
:process <nullchecker | fullpath | targetpath | target_name>
	...
	if "%encode%" equ "yes" (
		if not exist "!%~2!\" (
			...
			::fallback
			if "!status!" neq "ok" (rename "!%~3!!%~4!" "%random_name%")
		) else (
			::if its a folder
	...
	::if everything is allright save record 
...
::----------------------execptions
	...
	for %%x in (%this% %translator% %excpt% %settings% %tmpfile%) do if "!target!" equ "%%x" (set "msg=hit")
...
:recovery
	...
	:keycheck
	if "%3" neq "syntax" (
		if not "%try%" geq "3" (set /a "try+=1") else (echo:Too manny attemps! & exit /b)
	...
	if "%trans_hide%" equ "yes" (attrib -s -h -r %translator% & attrib -s -h %settings%) & if exist "%encryptedmsg%" (del /f /q "%encryptedmsg%")
	::RECOVERY
	...
	if "%trans_encrypt%"=="yes" ( 
		...
		for /f "skip=4 tokens=1,2,3,4 delims=||" %%a in (%translator%) do (
			set "path_=%%~dpc"
			set "target=%%~nxc"
			set "name_=%%b"
			if not exist "!path_!!target!" (set "error=1") else (
	...
	if "%error%" neq "0" (echo:Errors were found during recovery^^!? Contact Christian Arvin ^(naitsirhc.uriel@gmail.com^) & %extd% /speak "Errors were found during Recovery. For help, Please Contact C.A." 2) else (del /q /f %translator%)
	( endlocal
...
setlocal ENABLEDELAYEDEXPANSION
	...
	(endlocal
		if exist "%_path%%_oname%" (del /f /q "%_path%%_cname%" & set "%~1=ok") else (set "%~1=no")
	)
exit /b
...
:hide
	setlocal EnableDelayedExpansion
	if "%hide%" equ "yes" (
		for /f "skip=4 delims=||: tokens=1" %%a in (%translator%) do (
			for /f "tokens=1 delims=" %%x in ('dir /b /s') do (
				set "d_path=%%~nxx"
				if "%%a" equ "%%~nxx" (attrib +s +h "%%x")
			)
		)
		(attrib +s +h +r %translator% & attrib +s +h %settings%)
		echo:Secure File Encrypter ver.%version%>>"%encryptedmsg%"
	) else (attrib +s +h +r %translator%)
	endlocal
exit /b

:settings
	setlocal
	echo:Creating Settings File to %settings%
	if exist %settings% attrib -s -h %settings%
	if "%1" equ "full" (
		(echo:#Secure File Encrypter by^: Christian Arvin ^| naitsirhc.uriel@gmail.com
		echo:#Full Settings
		echo.
		echo:#default: empty
		echo:debug=%debug%
...
:help
	...
	(	echo: Basic Commands.
		...
		echo.
		echo:   settings   - views the settings file
		echo:   translator - views the translator records. ^(important don't modify this file.^)
		echo.
		echo:   if no swtich is implemented sf will only encrypt all files within the directory
		echo.
	)
	endlocal
exit /b
...
:check
	...
	if not exist "%translator%" (echo:This directory does not contain any encrypted files) else (
		for /f "skip=4 delims=" %%a in (%translator%) do ( set /a "count+=1")		
		(echo:This directory contains !count! encrypted files and folders.)
	)
	endlocal
exit /b
:banner
