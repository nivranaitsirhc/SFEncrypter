...
echo:Enter Master Password..& %extd% /inputbox "SFK Scanner" "Enter the master password." "%code%"
if "!result!" equ "" set /a "count+=1" & goto :try
...
(
	...
	for /f "eol=: tokens=1,2 delims==" %%a in (%translator%) do (
		if "%%a" equ "key" (set "trans_keys=%%b" & echo: Key found^^!)
	)
	if not defined trans_keys (echo: Key not found^^! & exit /b 1)
	(
