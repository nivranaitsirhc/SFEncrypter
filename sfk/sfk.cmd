...
title Secure File Encrypter Key Scanner
cls
echo.
...
(
	if not exist %translator% (
		echo: Translator missing.^!?
		if defined kustom (echo: Are your sure thats the right translator?)
		exit /b 1
	) else (echo: Translator file found^^!)
)
if "%~1" equ "" (set "username=1001") else if "%~1" equ "1001" (set "username=%username%")
:try agian
if "%count%" geq "3" echo: Too many retries..&exit /b 1
echo: Enter Master Password..
:redo
if "%nullcount%" geq "5" echo: Quit playing with me..&exit /b 1
%extd% /inputbox "SFK Scanner" "Enter the master password." "%code%"
if "!result!" equ "" (set /a "nullcount+=1" &goto :redo)
if "%username%" neq "!result!" set /a "count+=1" & goto :try
:skip
if defined kustom (echo: Using Kustom Translator "%kustom%" & set "translator=%1")
(
	setlocal enableDelayedExpansion
	echo: Scanning key....
	for /f "eol=: tokens=1,2 delims==" %%a in ('type %translator% ^| findstr key') do (
		if "%%a" equ "key" (set "trans_keys=%%b")
	)
	if not defined trans_keys (echo: Key not found^^! & exit /b 1) else echo: Key found^^!
	(
		echo: Decrypting   :!trans_keys!
		call :encrypt trans_keys trans_keys
		echo: encrypKey is :!trans_keys!
		%extd% /inputbox "Secure File Key Unlocker" "Your translator key is:" "!trans_keys!"
		%extd% /setclipboardtext "!trans_keys!"
